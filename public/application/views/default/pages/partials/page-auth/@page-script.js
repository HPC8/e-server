...
//  users auth
jQuery(document).on('click', 'button#auth-submit', function () {
    jQuery.ajax({
        type: 'POST',
        url: baseurl + 'users/authSubmit',
        data: jQuery("form#auth-form").serialize(),
        dataType: 'json',
        ...
        success: function (json) {
            ...
            if (json['error']) {
                ...
                jQuery('form#auth-form').find('textarea, input').each(function () {
                    jQuery(this).val('');
                });
                jQuery('#auth-submit').modal('hide');
